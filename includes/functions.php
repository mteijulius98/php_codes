<?php
//this file is going to include all basic 
function mysql_prep($value){
    ...
    if($new_enough_php){ //PHP v4.3.0 or higher
       ...
       if($magic_quotes_active) { $value = stripslashes($value);}
        ...
        if(!$magic_quotes_active){$value = assslashes($value);}
...
function navigation ($sel_subject,$sel_page,$public=false){
     $output = "<ul class=\"subjects\">";             $subject_set =  get_all_subjects($public=false);       
...
function public_navigation($sel_subject,$sel_page, $public = true){
    $output = "<ul class=\"subjects\">";             $subject_set =  get_all_subjects($public);       
while($subject = mysql_fetch_array($subject_set)){
	$output .= "<li";
    if($subject["id"]== $sel_subject['id']){ $output .= "  class=\"selected\""; }
